...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingServiceImpl(BookingRepository bookingRepository, UserService userService, ItemService itemService, BookingMapper bookingMapper) {
    ...
    public Booking approveBooking(Long userId, Long bookingId, Boolean approved) {
        ...
        if (!booking.getItem().getOwner().getId().equals(userId)) {
            throw new NotValidException("Вы не являетесь владельцем вещи");
        }
        if (approved) {
